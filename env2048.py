...
class Env2048(object):
    ...
    def to_tensor(self):
        return state2tensor(self.state_)
    ...
    def add_random_tile(self):
        ...
        if avilable.size:
            self.state_.itemset(
                np.random.choice(avilable), 1
                if np.random.random() < 0.9 else 2)
            return False
        else:
            return True
    ...
    def move(self, direct):
        ...
        if direct == 0:  # up
            ...
            for i in range(4):
                self.state_[-1::-1, i], c = self.emit_line(
                    self.state_[-1::-1, i])
                changed = changed or c
        elif direct == 2:  # left
            for i in range(4):
            ...
            for i in range(4):
                self.state_[i, -1::-1], c = self.emit_line(
                    self.state_[i, -1::-1])
                changed = changed or c
        return changed
